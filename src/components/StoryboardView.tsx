  ...
  const handleAssetUpload = (e: React.ChangeEvent<HTMLInputElement>, key: string) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        if (key === 'productAsset') newState.productAssets = [...(newState.productAssets || []), newAsset];
        else if (key === 'characterAsset') newState.characterAssets = [...(newState.characterAssets || []), newAsset];
        else newState[key] = newAsset;
  ...
  const generateAIPrompt = async () => {
    ...
    try {
      ...
      const prompt = `
        ...
        CHARACTER: ${editorState.characterAssets?.length ? 'Provided' : 'None'}
          ...
          <div className="bg-white/[0.02] border border-white/5 rounded-3xl p-6 md:p-8 space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-7 gap-4">
              {[
                ...
                let assetData = asset.key === 'productAsset' ? editorState.productAssets?.[0] : 
                               asset.key === 'characterAsset' ? editorState.characterAssets?.[0] : 
                ...
                <div key={asset.label} className="space-y-2">
                  ...
                  {asset.type === 'text' ? (
                    ...
                    asset.key === 'ciPdfAsset' ? (
                        ...
                        <img src={`data:${assetData.mimeType};base64,${assetData.data}`} className="object-contain w-full h-full" />
                    )
