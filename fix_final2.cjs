...
const regex = /\{generationObjective !== 'logo' && \(\s*<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">[\s\S]*?(<div className="space-y-2">\s*\{generationObjective !== 'logo' && \(\s*<>\s*<h3 className="text-xs font-bold text-white\/60 uppercase tracking-wider">Character\/Model<\/h3>)/;

const match = content.match(regex);
if (match) {
    console.log("Found match");
    const replacement = `\{generationObjective !== 'logo' && (
    ...
    fs.writeFileSync(file, content.replace(match[0], replacement));
}
